...
"""Dense tail Liquid Legions cardinality estimator prototype."""
...
class DenseTailLiquidLegions(base.SketchBase):
  """DenseTailLiquidLegions sketch."""
  ...
  def add_fingerprint(self, f):
    ...
    b = self.get_bucket(f)
    # This is simulation of DenseTailLiquidLegions same-key-aggregator
  ...
  def kth_inactive_register(self, k):
    """Return the first inactive register, i.e, count is 0."""
    cnt = 0
    for i in range(self.m):
      if i not in self.sketch:
        cnt += 1
        if (cnt >= k):
          return i
    return -1

  def kth_nondestroyed_register(self, k):
    """Return the first nondestroyed register, i.e, unique is not -1."""
    cnt = 0
    for i in range(self.m):
      if i not in self.unique:
        cnt += 1
      elif self.unique[i] != -1:
        cnt += 1
      if (cnt >= k):
        return i
    return -1
  ...
  def get_compatible_sketch(self):
    ...
    return DenseTailLiquidLegions(a=self.a, m=self.m, random_seed=self.seed)


class Noiser(cascading_legions.Noiser):
  """Noiser of DenseTailLiquidLegions."""
...
class Estimator(base.EstimatorBase):
  """Estimator for DP-noised DenseTailLiquidLegions."""
  ...
  def sublegion_as_vector(cls, sketch_list, start, m):
    ...
    Args:
      sketch_list: A list of DenseTailLiquidLegions.
...
class SequentialEstimator(base.EstimatorBase):
  """Estimator for DenseTailLiquidLegions via sequential merge."""
